...
from oas_data_utils import ABSADataset, task_data_list, cal_entropy
from oas_const import *
from oas_data_utils import read_line_examples_from_file, force_tokens, cate_list, force_words
from oas_eval_utils import compute_scores, extract_spans_para
...
def init_args():
    ...
    if not os.path.exists('./outputs/oas'):
        os.mkdir('./outputs/oas')
...
def reorder_group(group_data):
    return [group_data[1], group_data[2], group_data[0]]
    ...
    if args.do_inference:
        ...
        with open(log_file_path, "a+") as f:
            ...
            if args.multi_task:
                ...
                result_txt = './outputs/oas/result_oas.txt'
                result_label = './outputs/label/result_label.txt'
                with open(result_label, 'w') as file:
                    for i in range(len(all_labels)):
                        file.write(str(all_labels[i]))
                        file.write('\n')
                file.close()
