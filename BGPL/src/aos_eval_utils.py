...
def compute_f1_scores(pred_pt, gold_pt, verbose=True):
    ...
    result_aos = './outputs/aos/result_aos.txt'

    with open(result_aos, 'w') as file:
        for i in range(len(pred_pt)):
            file.write(str(pred_pt[i]))
            file.write('\n')
    file.close()
...
def compute_scores(pred_seqs, gold_seqs, final_calculation, verbose=True):
    """
...
#            print()
    ...
    for i in range(len(final_calculation)):
        filter_list = []
        for j in range(len(final_calculation[i])):
            if float(final_calculation[i][j].strip('[]')) < 0.4:
                filter_list.append(int(j/3))
        if filter_list:
            filter_list = list(set(filter_list))
            for k in range(len(filter_list)-1, -1, -1):
                del final_calculation[i][3*k:3*k+3]
                del all_preds[i][k]
